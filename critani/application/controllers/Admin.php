...
class Admin extends CI_Controller
{
    ...
    public function about_critani()
    {
        $data['user'] = $this->db->get_where('admin', ['email' =>
            $this->session->userdata('email')])->row_array();

        $this->load->view('admin/about_critani');
    }
    ...
    public function delete_user($id)
    {
        $this->load->model('m_user');
        $where = array('id' => $id);
        $this->m_siswa->delete_user($where, 'user');
    ...
    public function data_mitra()
    {
        ...
        $data['user'] = $this->m_mitra->tampil_data()->result();
        $this->load->view('admin/data_mitra', $data);
    }

    public function detail_mitra($nip)
    {
        $this->load->model('m_mitra');
        $where = array('nip' => $nip);
        $detail = $this->m_mitra->detail_mitra($nip);
        $data['detail'] = $detail;
        $this->load->view('admin/detail_mitra', $data);
    }

    public function update_mitra($nip)
    {
        $this->load->model('m_mitra');
        $where = array('nip' => $nip);
        $data['user'] = $this->m_mitra->update_mitra($where, 'mitra')->result();
        $this->load->view('admin/update_mitra', $data);
    }

    public function mitra_edit()
    {
        $this->load->model('m_mitra');
        ...
        $this->m_mitra->update_data($where, $data, 'mitra');
        $this->session->set_flashdata('success-edit', 'berhasil');
    ...
    public function materi_edit()
    {
        ...
        $nama_cerita = $this->input->post('nama_cerita');
        $deskripsi = $this->input->post('deskripsi');

        $data = array(
            'nama_mitra' => $nama_mitrau,
    ...
    public function delete_mitra($nip)
    {
        $this->load->model('m_mitra');
        $where = array('nip' => $nip);
        $this->m_gmitra->delete_mitra($where, 'mitra');
        $this->session->set_flashdata('user-delete', 'berhasil');
        redirect('admin/data_mitra');
    }
    ...
    public function add_mitra()
    {
        ...
        if ($this->form_validation->run() == false) {
            $this->load->view('mitra/registration');
        } else {
            $data = [
                ...
                'nama_cerita' => htmlspecialchars($this->input->post('cerita', true)),
            ];

            $this->db->insert('mitra', $data);

            $this->session->set_flashdata('success-reg', 'Berhasil!');
            redirect(base_url('admin/data_mitra'));
        }
    }
    ...
    public function tambah_materi()
    {
        ...
        if ($this->form_validation->run() == false) {
            ...
            $data = [
                'nama_mitra' => htmlspecialchars($this->input->post('nama_gmitrau', true)),
                'nama_cerita' => htmlspecialchars($this->input->post('nama_cerita', true)),
                'video' => $video,
