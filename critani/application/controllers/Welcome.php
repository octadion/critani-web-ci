...
class Welcome extends CI_Controller
{
    ...
    public function critani()
    {
        $this->load->view('template/nav');
        $this->load->view('critani');
        $this->load->view('template/footer');
    }
    ...
    public function mitra()
    {
        ...
        if ($this->form_validation->run() == false) {
            ...
            //validasi sukses
            $this->mitra_login_process();
        }
    }

    private function mitra_login_process()
    {
        $email = $this->input->post('email');
        $password = $this->input->post('password');

        $user = $this->db->get_where('mitra', ['email' => $email])->row_array();
        ...
        if ($user) {
            ...
            if (password_verify($password, $user['password'])) {
                ...
                $this->session->set_flashdata('fail-pass', 'Gagal!');
                redirect(base_url('welcome/mitra'));
            }
        } else {
